...
This same fixed TF4 checkpoint (never retrained) was reused to regenerate four additional S1 evaluation bundles, one per new pre-declared spatial partition (grid 0.5°/1.0°/2.0°, seeds 7/42/123), each scoped to that partition's own spatial-test observations and independently passing `s1_bundle.py`'s leakage/provenance validation against that partition's own split identity (never this bundle's). S1-only diagnostic accuracy stayed in a comparatively narrow 89.8%-91.75% range across all five partitions (this one included), while fixed fusion with each partition's own independently-trained geographic prior outperformed S1-only on every partition. Full per-partition results are in `docs/m2d_robustness_report.md`. This card's own bundle, checkpoint, and limitations above are unchanged.
